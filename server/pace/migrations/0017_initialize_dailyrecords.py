    ...
    def forwards(self, orm):
        for classroom in orm.Classroom.objects.all():
            ...
            for student in classroom.students.all():
                for periodic_record in student.periodic_records.all():
                    pd_set = date_record_map.setdefault(periodic_record.date, set())
                    pd_set.add(periodic_record.period)

            for date, pd in date_record_map.items():
                orm.DailyRecord.objects.create(classroom=classroom, date=date, current_period=max(pd))
